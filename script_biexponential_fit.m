...
variables = {'sample','channel','t0','num_clones','num_singlets','avg_clone_size'...
...
for nsample = samples
    ...
    for nchannel = channels
        ...
        avg_clone_size = mean(sample_clones);
        ...
        % average clone size (all clones)
        result_table.avg_clone_size(counter) = avg_clone_size;
...
if show_plots == 1 % plot sensitivity to variations of "r", Extended Data Figure
    figure;
    r_ref = 0.75;
    legs = {}; 
    ...
    for i = 1:3
        subplot(1,3,i)
        hold on; add_vref_line(r_ref); hold off;
    end
end
%%
% save results
...
ys = ylim();
xs = [x,x];
plot(xs,[0,ys(2)],'k--','HandleVisibility','off')
end
